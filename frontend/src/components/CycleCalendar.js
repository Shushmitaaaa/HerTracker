...
const CycleCalendar = ({ lastPeriodDate, periodLength }) => {
  ...
  const isPeriodDay = (currentDate) => {
    ...
    // Period ki range: Start Date se lekar (Start Date + Period Length) tak
    end.setDate(start.getDate() + (periodLength || 5));

    // Time reset kar rahe hain taaki sirf date compare ho
  ...
  const tileClassName = ({ date, view }) => {
    if (view === 'month' && isPeriodDay(date)) {
      return 'period-highlight'; // Is class ko CSS mein red dot ya highlight ke liye use karo
    }
    return null;
  };
